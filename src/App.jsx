      ...
      <main className="max-w-4xl mx-auto px-6 py-20">
        ...
        <div className="grid gap-8">
          {essays.map((essay) => {
            ...
            const date = essay.date || 'Date pending';
            const title = essay.title || 'Untitled essay';
            const excerpt = essay.excerpt || 'No summary is available yet.';
            const author = essay.author || 'Unknown author';
            const readTime = essay.readTime || 'Read time unavailable';
            const likes = Number.isFinite(essay.likes) ? essay.likes : 0;

            return (
              <article
                key={essay.id}
                className="group relative bg-white/60 backdrop-blur-sm rounded-3xl p-8 md:p-10 border border-white/50 shadow-sm hover:shadow-xl hover:bg-white/80 transition-all duration-300 hover:-translate-y-1 cursor-pointer"
              >
                <div className="flex items-center gap-3 text-sm font-bold tracking-wider text-indigo-500 mb-4">
                  <span className="bg-indigo-50 px-3 py-1 rounded-full">{category}</span>
